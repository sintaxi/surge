...
var fetchAccount = exports.fetchAccount = function(endpoint){
  ...
  return function(email, pass, callback){
    var url = new URL('/account', endpoint.format())
    var authorization = `Basic ${
      Buffer.from(`token:${req.creds.token}`).toString('base64')
    }`
    var options = {
      headers: { version: pkg.version, authorization },
    }
    fetch(url, options).then(async function(r, obj){
      if (r.status == 200){
...
var fetchToken = exports.fetchToken = function(endpoint){
  ...
  return function(email, pass, callback){
    ...
    var url = new URL('/token', endpoint.format())
    var authorization = `Basic ${
    ...
    fetch(url, options).then(async function(r){
      if(r.status == 417){
        ...
        return callback(await r.json(), null)
      }else{
        try {
          var o = await r.json()
          localCreds(endpoint).set(o.email, o.token.replace(os.EOL, ""))
          ...
          if (count >=3) {
            req.read({
              ...
              if (reply == "yes" || reply == "y" || reply == "Y") {
                var url = new URL(`/token/reset/${req.email}`, req.endpoint)
                var options = {
                  'method': 'POST'
                }
    ...
    read({
      ...
      read({
        ...
        read({
          ...
          form.set('card[exp_month]', ep.split("/")[0])
          form.set('card[exp_year]', ep.split("/")[1])
          form.set('card[cvc]', cv)
          var url = "https://"+ stripe_pk +":@api.stripe.com/v1/tokens"
          var options = { methdo: 'POST', body: form }
          fetch(url, options).then(async function(r) {
            if (r.status == 417){
              space()
              trunc("Update Required".yellow, "-", await r.text())
              space()
              process.exit(1)
            } else if (r.status == 401){
              console.log(r)
