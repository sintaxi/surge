...
exports.payment = function(req, stripe_pk, existing){
  ...
  var abort = function(msg){
    ...
    if (req.plan){
      ...
      console.log("   " + "Aborted".yellow + " - Using existing card.".grey)
    }
    console.log()
    process.exit(1)
  }
  ...
  if (existing !== null) {
    ...
    return function(callback){
      ...
      var prompt = req.plan
        ? ("   Change to the " + req.plan.underline + " plan using " +  existing.brand.blue + " ending in ".grey + existing.last4.blue + "?".grey).grey
        : ("   Continue to use " +  existing.brand.blue + " ending in ".grey + existing.last4.blue + "?".grey).grey
