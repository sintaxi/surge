...
module.exports = function(req, next){
  ...
  .on("regionInfo", function(payload){
    helpers.displayRegions(payload)
  })
  ...
  .on("collect", function(payload){
    ...
    helpers.payment(req, payload.stripe_pk, payload.card)(function(paymentToken){
      sdk.plan({ 
        ...
        if (errors) {
          helpers.log("error".red + " - troubles switching plan".grey)
        }else{
          helpers.reset()
          helpers.space()
  ...
  .on("success", function(){
    helpers.space()
    helpers.log("   Success!".green + (" - Published to " + (req.domain).underline).grey)
    helpers.space()
    return next()
  })
