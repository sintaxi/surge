  ...
  var tick = function(tick){
    ...
    //try {
      ...
      if (payload.hasOwnProperty("type") && payload.type === "collect") {
        helpers.log()
        helpers.log(("      Project requires a ".blue + ("$" + (payload.plan.amount / 100) + "/mo").yellow + " upgrade (cancel anytime). This unlocks:".blue))
        helpers.log("          - Custom SSL".blue)
        helpers.log("          - Force http to https".blue)
        helpers.log("          - CORS".blue)
        helpers.log("          - Customer Redirects".blue)
        helpers.log("          - Analytics".blue)
