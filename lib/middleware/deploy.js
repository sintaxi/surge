  ...
  var tick = function(tick){
    ...
    //try {
      ...
      if (payload.hasOwnProperty("type") && payload.type === "event_registration") {
        if (payload.data) {
          console.log()
          console.log(("      " + payload.data.event.name + " is set and ready to go!").bold)

          if (payload.data.event.publishing_window){
            console.log("              start:".grey, payload.data.event.publishing_window[0])
            console.log("                end:".grey, payload.data.event.publishing_window[1])
          }
      ...
      if (payload.hasOwnProperty("type") && payload.type === "event_participant") {
        if (payload.data) {
          console.log()
          console.log(("      You are participating in " + payload.data.event.name).bold + "!")
