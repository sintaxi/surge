...
module.exports = function(req, next, abort){
  ...
  var auth = function(){
    ...
    req.read({
      ...
      if (password === undefined) return abort("Not authenticated.".grey)
            ...
            req.read({
              ...
              if (reply == "yes" || reply == "y" || reply == "Y") {
                ...
                request(options, function(e, r, obj){
                  ...
                  if (r.statusCode == 201) {
                    console.log()
                    helpers.trunc("Password Recovery".yellow + " - reset instructions sent to".grey, req.email.green)
                    console.log()
                    process.exit(1)
                  } else {
                    console.log()
                    helpers.trunc("Oops".red + " - something went wrong trying to reset your password.".grey)
                    console.log()
                    process.exit(1)
                  }
                })
                ...
                console.log()
                helpers.trunc("Aborted".yellow + (" - No password reset sent to " + req.email.underline + ".").grey)
                console.log()
                process.exit(1)
              }
