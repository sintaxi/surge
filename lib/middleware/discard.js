    ...
    sdk.delRevision(domain, rev, { user: "token", pass: req.creds.token }, function(error, response){
      if (error){
        ...
        if (response.status && response.status == 200){
          ...
          helpers.space()
          helpers.log("   ✂ Discarded".green + (" " + [response.rev, response.domain].join(".").underline).grey)
          helpers.space()
          ...
          if (response.uncached.change){
            ...
            if (response.uncached.change == "rollfore"){
              ...
              helpers.log("   Done".green + (" - " + response.domain.underline + " rolled forward to revision " + [response.revision.rev, response.domain].join(".").underline).grey)
            } else if (response.uncached.change == "rollback"){
              var revDomain = [response.revision.rev, domain].join(".")
              helpers.log("   Done".green + (" - " + response.domain.underline + " rolled back to revision " + [response.revision.rev, response.domain].join(".").underline).grey)
            } else if (response.uncached.change == "offline"){
              helpers.log("   Done".green + (" - No revisions to serve. Project now offline.").grey)
            } 
            helpers.space()
          } else {
            helpers.log("   Done".green + (" - " + response.domain.underline + " still serving " + [response.revision.rev, response.domain].join(".").underline).grey)
          }
