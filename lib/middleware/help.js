...
module.exports = function(req, next){
  if (req.argv.help || req.argv.h) {
    ...
    // contextual - a domain passed in beats one discovered in the cwd,
    // and the lines render as exactly what to type
    var local = null
    ...
    if (!local) local = discovery.local()
    ...
    if (local){
      helpers
        .log("  " + local.domain.underline + (" (" + (sourceLabels[local.source] || local.source) + ")").grey)
        .log()
    }
