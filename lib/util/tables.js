...
var dcTrend = function(s){
  ...
  if (full.slice(-3).reduce(function(a, b){ return a + b }, 0) === 0) return "✕"
...
var buildLoadChart = function(datacenters, total){
  ...
  var points = dcs.map(function(dc){
    return {
      label: padRight(dc.name, nameLen) + "  " + padRight(dc.city, cityLen) + "  " + dc.trend,
      value: dc.t
    }
  })
  ...
  var chart = new Chartscii(points, {
    ...
    valueLabelFormat: function(values){
      return values.map(function(v){
        ...
        return hrn(Number(v)) + (" · " + share + "%")
      }).join("")
    }
  }).create()

  var legend = "↗ ↘ vs previous week".grey
  if (dcs.some(function(dc){ return dc.trend === "✕" })) legend += "   ✕ no traffic 3+ days".grey
  return chart + "\n" + legend
}
...
var buildRegionLine = function(datacenters, total){
  ...
  return Object.keys(regions).sort(function(a, b){ return regions[b] - regions[a] })
    .map(function(region){
      return region.white + (" " + Math.round(regions[region] / total * 100) + "%").grey
    }).join("   ")
}
...
exports.load = function(data){
  ...
  table.push([{
    hAlign: "left",
    content: hrn(total).white + (" connections / " + data.range.length + " days   ").grey + buildRegionLine(data.datacenters, total)
  }])
...
exports.traffic = function(data){
  ...
  var uniquesDelta      = (uniquesYesterday - uniquesAverage) / uniquesAverage * 100
