...
var displayServers = exports.displayServers = function(servers){
  var table = tables.servers(servers)
  var rows  = table.toString().split("\n")
  //log(smart("CNAME:").grey + " dns.surge.sh " + "(or use NS servers below)".grey)
  //space()
  //log("   Preview at ".grey + "https://123456789.sintaxi.com".grey.underline)
...
var displayProps = exports.displayProps = function(props, args){
...
var displayCtas = exports.displayCta = function(payload, args){
  space()
  if (payload.ctas){
    var table = tables.ctas(payload.ctas, args.colWidth, payload.cta_color || "grey")
    var rows = table.toString().split("\n")
    rows.forEach(function(row, i){
      log(" ", row)
    })
...
exports.displayPublishInfo = function(payload){
  ...
  if (payload.props) 
    displayProps(payload.props, { short: true })

  var table      = tables.servers(payload.servers)
  var rows       = table.toString().split("\n")
  ...
  if (payload.ctas) 
    displayCtas(payload, { short: true, showEmpty: false, colWidth: tableCols })
...
exports.revisionSelector = function(req, callback){
  ...
  request(options, function(e, r, obj){
    ...
    if (list.length == 0){
      space()
      trunc("Empty".blue + (" - There are no revisions for " + req.argv["_"][0]).grey)
      ...
      inquirer.prompt([args]).then(function (answers) {
        if (answers.revision.current){
          trunc("No change".green + " - Selected revision is already current".grey )
          return next()
        } else {
