...
exports.displayRevisionBasicInfo = function(revision, domain){
  space()
  trunc(("  " + [revision.rev, domain].join(".").underline).grey)
  trunc(("  " + revision.fileCount + " files, " + revision.totalSize + " size").grey)
  trunc(("  " + revision.email).grey)
  // trunc(("  " + [revision.rev, domain].join(".").underline).yellow + " " + revision.email.grey)
  // trunc(("  " + revision.fileCount + " files, " + revision.totalSize + " size").grey)
  space()
}
...
exports.displayRegions = function(payload){
  var table = tableRegions(payload)
...
exports.defaults = {
  404: function(e, r, b){
    space()
    trunc("Error".red + " - Project Not Found".grey)
    space()
    process.exit(1)
  }
}