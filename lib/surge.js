...
module.exports = function(configuration){
  ...
  var surge = function(args){
    ...
    if (cmd && commands.indexOf(cmd) !== -1) {
      ...
      var legacyTarget = t && cmd !== "publish" && targetable.indexOf(cmd) !== -1 && (isDomainish(t) || isPathish(t)) ? t : null
    ...
    if (cmd){
      ...
      if (domainT || pathT){
        ...
        if (verb === "publish"){
          if (domainT){
            return abort("publish takes a path - try `" + configuration.name + " <path> " + cmd + "`.")
          }
          argv._.splice(1, 1)
          return surge.publish({})(argv)
        }
