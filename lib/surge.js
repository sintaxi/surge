...
module.exports = function(config){
  ...
  var surge = function(args){
    ...
    var commands = ["login", "logout", "whoami", "list", "publish", "teardown", "token"]
  ...
  surge.teardown = function(hooks){
    ...
    var onion = [
      whitelist, endpoint, pkg, help, version, space,
      preAuth, creds, welcome, tokencheck, email, auth, postAuth,
      discovery.suggestDomainFromCname, domain,
      function(req, next){
        console.log(req.project, req.domain)
        next()
      }, space
    ]
    return function(){
  ...
  surge.pro = function(hooks){
    var preAuth   = hooks.preAuth  || stub
    var postAuth  = hooks.postAuth || stub
    var onion = [
      whitelist, endpoint, pkg, help, version, space,
      preAuth, creds, welcome, tokencheck, email, auth, postAuth, space
    ]
    return function(){
      var argv = parse(arguments[arguments.length -1])
      skin({
