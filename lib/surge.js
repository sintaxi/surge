...
module.exports = function(configuration){
  ...
  var surge = function(args){
    ...
    // points at where they live now. ssl, zone, and token stay silent:
    // hidden capabilities, not deprecations.
    var nudges = {
      status:    "debug",
      files:     "debug files",
      audit:     "debug audit",
    ...
    if (cmd && commands.indexOf(cmd) !== -1) {
      ...
      if (cmd === "token" && (String(argv._[0]) === "add" || String(argv._[0]) === "rem")) nudge = "tokens " + argv._[0]
      if (cmd === "list" && argv._[0] != null) nudge = "revs"
      if (nudge){
        helpers.space()
        helpers.trunc(("`" + configuration.name + " " + cmd + "` is now `" + configuration.name + " " + nudge + "`").grey)
      }
    ...
    if (cmd === "plus"){
      helpers.space()
      helpers.trunc("Aborted".yellow + (" - the Plus plan is legacy. Manage your plan with `" + configuration.name + " plan`.").grey)
      helpers.space()
  ...
  surge.dns = function(hooks){
    ...
    return function(){
      ...
      // bare noun shows its usage - rendered against the local domain
      if (!a0) return help.dnsUsage(configuration.name, discovery.local())

      // infer the domain when the verb leads
      ...
      } else if (!helpers.validDomain(a0)){
        // a path in the domain slot resolves like everywhere else
        var found = discovery.local(a0)
        if (!found){
          var msg = fs.existsSync(a0)
            ? "No CNAME found in " + a0 + " - pass a domain or publish first."
            : "`" + a0 + "` is not a domain."
          helpers.space()
  ...
  // the [<domain>] slot. returns the default verb for domain-only input,
  // null for a bare call (show the noun's usage - discovery first), and
  ...
  surge.debug = function(hooks){
    ...
    return function(){
      ...
      if (verb === null) return help.debugUsage(configuration.name, discovery.local())
      return surge[verb](hooks || {})(argv)
    }
  }

  // stats namespace - [<domain>] traffic•load•audience•usage, bare = traffic
  surge.stats = function(hooks){
    var verbs = ["traffic", "load", "audience", "usage"]
    return function(){
      var argv = parse(arguments[arguments.length -1])
      var verb = namespaceVerb(argv, verbs, "stats", "traffic")
      if (verb === null) return help.statsUsage(configuration.name, discovery.local())
      return surge[verb](hooks || {})(argv)
    }
  }
