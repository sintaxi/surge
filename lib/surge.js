...
var exitifcurrentplan = function(req, next){
  //"\n               plan: ".grey + plan.name.default

  if (req.subscription.plan.id === req.selectedPlan.plan){
    console.log("               plan: ".grey + req.subscription.plan.name)
    console.log()
    process.exit()
  }else{
    return next()  
...
module.exports = function(config){
  ...
  var surge = function(args){
    ...
    var commands = ["login", "logout", "whoami", "list", "publish", "teardown", "token", "plus", "ssl", "plan"]
  ...
  surge.plan = function(hooks){
    ...
    var onion = [
      whitelist, endpoint, pkg, help, version, space,
      preAuth, creds, tokencheck, email, auth, postAuth,
      subscription, plans, plan, exitifcurrentplan,
      space
    ]
    return function(){
      var argv = parse(arguments[arguments.length -1])
