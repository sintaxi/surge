...
module.exports = function(configuration){
  ...
  var surge = function(args){
    ...
    if (cmd && commands.indexOf(cmd) !== -1) {
      ...
      var legacyTarget = t && targetable.indexOf(cmd) !== -1 && (isDomainish(t) || isPathish(t)) ? t : null
      if (cmd === "token" && (t === "add" || t === "rem")) nudge = "tokens " + t
      if (cmd === "list" && legacyTarget) nudge = "revs"
      if (nudge || legacyTarget){
        var typed = cmd + (legacyTarget ? " " + legacyTarget : "")
    ...
    if (cmd){
      ...
      if (domainT || pathT){
        ...
        // the publish pair - surge <path> <domain>
        if (pathT && verb && isDomainish(verb)){
          return surge.publish({})(argv)
        }

        // surge <proj> publish - the constructor, no project required yet
        if (verb === "publish"){
          argv._.splice(1, 1)
          if (domainT) argv._.unshift(".")
          return surge.publish({})(argv)
        }

        // any other use of a path target requires a surge project
        if (pathT && !discovery.local(cmd)){
          return abort("`" + cmd + "` is not a surge project - publish first or pass a domain.")
        }

        // verbless never acts - overview in a terminal, loud in a pipeline
        if (!verb){
          if (!process.stdout.isTTY && !argv.help && !argv.h){
            console.error(configuration.name + ": nothing to do - try `" + configuration.name + " " + cmd + " publish` or `" + configuration.name + " " + cmd + " <command>`")
            return process.exit(1)
          }
          argv.help = true
          return skin({
  ...
  var targetContext = function(typed){
    ...
    if (!found){
      helpers.space()
      helpers.trunc("Aborted".yellow + (" - `" + typed + "` is not a surge project - publish first or pass a domain.").grey)
      helpers.space()
      return process.exit(1)
    }
